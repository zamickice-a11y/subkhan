...
(function () {
  ...
  function wireLineItems(data) {
      ...
      tbl.querySelectorAll("tbody tr").forEach(tr => {
        ...
        tr.querySelectorAll("input").forEach(inp => {
          inp.oninput = () => {
            ...
            // Auto-calc GST 10% if business is GST-registered
            const gstReg = window.SD_CONFIG && window.SD_CONFIG.BUSINESS && window.SD_CONFIG.BUSINESS.gst_registered;
            if (gstReg && (inp.dataset.f === "qty" || inp.dataset.f === "unit")) {
              const g = (Number(data.items[i].qty||0) * Number(data.items[i].unit||0) * 0.10);
              data.items[i].gst = g.toFixed(2);
              const gInp = tr.querySelector('[data-f="gst"]'); if (gInp) gInp.value = data.items[i].gst;
            }
            const lineTot = Number(data.items[i].qty||0)*Number(data.items[i].unit||0) + Number(data.items[i].gst||0);
            tr.querySelector(".tot").textContent = "$" + window.SD_DOCGEN.money(lineTot);
            refreshTot();
          };
        });
    ...
    if (qi) qi.onchange = () => {
      ...
      const gstReg = window.SD_CONFIG && window.SD_CONFIG.BUSINESS && window.SD_CONFIG.BUSINESS.gst_registered;
      data.items.push({ qty: 1, desc: it.desc, unit: it.unit, gst: gstReg ? (it.unit * 0.10).toFixed(2) : 0 });
      redrawItems(data);
    };
    rebind(); refreshTot();
  }
